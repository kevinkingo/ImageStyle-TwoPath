...
'''
command = 'scp srthu2@103.41.53.119:'
styles = [2, 5, 6, 15, 16, 18]
command += '{'
for style in styles:
	command += style_pic[style][random.randrange(len(style_pic[style]))]
	if style != 18:
		command += ','
...
in_db = lmdb.open('flickr-train-lmdb', map_size=int(1e12))
...
in_db = lmdb.open('flickr-validate-lmdb', map_size=int(1e12))
with in_db.begin(write=True) as in_txn:
	for style in validate_pic:
...
in_db = lmdb.open('flickr-test-lmdb', map_size=int(1e12))
with in_db.begin(write=True) as in_txn:
	for style in test_pic:
