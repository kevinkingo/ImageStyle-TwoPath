...
path = 'prob/'
if save:
	net = caffe.Net('wiki_test_texture_model.prototxt', 'models/wiki_texture_iter_30000.caffemodel', caffe.TEST)
	...
	while True:
		...
		output = net.forward(blobs=['gram_fc3', 'label'])
		probs[count:count + num, :] = output['gram_fc3'].reshape(batch_num, classify_num)[:num, :]
	...
	np.save(path + 'wiki_test_texture.npy', probs)
	np.save(path + 'wiki_test_label.npy', labels)
else:
	probs = np.load(path + 'wiki_test_texture.npy')
