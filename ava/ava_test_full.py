...
path = 'prob/'
if save:
	net = caffe.Net('ava_test_full_model.prototxt', 'models/ava_full_iter_16000.caffemodel', caffe.TEST)
	...
	while True:
		...
		output = net.forward(blobs=['newfc8', 'gram_fc3'])
	...
	np.save(path + 'ava_test_full_object.npy', probs_object)
	np.save(path + 'ava_test_full_texture.npy', probs_texture)
	np.save(path + 'ava_test_label.npy', labels)
else:
	probs_object = np.load(path + 'ava_test_full_object.npy')
	probs_texture = np.load(path + 'ava_test_full_texture.npy')
