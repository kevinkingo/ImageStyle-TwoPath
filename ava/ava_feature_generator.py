...
caffe.set_mode_gpu()
crop_number = 8
train_num = 9813 * crop_number
validate_num = 1457 * crop_number
test_num = 2573 * crop_number
batch_num = 16

#Train
net = caffe.Net('ava_texture_model.prototxt', 'models/VGG_ILSVRC_19_layers.caffemodel', caffe.TRAIN)
print "Training feature total: " + str(train_num)
in_db = lmdb.open('data/ava-train-feature-lmdb', map_size=int(1e12))
...
#Validate
net = caffe.Net('ava_texture_model.prototxt', 'models/VGG_ILSVRC_19_layers.caffemodel', caffe.TEST)
print "Validation feature total: " + str(validate_num)
in_db = lmdb.open('data/ava-validate-feature-lmdb', map_size=int(1e12))
with in_db.begin(write=True) as in_txn:
	count = 0
	while True:
		output = net.forward(blobs=['gram_concat', 'label'])
